...
/*!50001 CREATE TABLE `view_pedidosproveedoresitems` (
  ...
  `proyectosespeciales_idproyectosespeciales` tinyint NOT NULL,
...
--
-- Temporary table structure for view `view_porcentajesreservas`
--

DROP TABLE IF EXISTS `view_porcentajesreservas`;
/*!50001 DROP VIEW IF EXISTS `view_porcentajesreservas`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE TABLE `view_porcentajesreservas` (
  `iditem` tinyint NOT NULL,
  `editorial_ideditorial` tinyint NOT NULL,
  `categoria_idcategoria` tinyint NOT NULL,
  `bodega_idbodega` tinyint NOT NULL,
  `usergroups_user_id` tinyint NOT NULL,
  `proyectosespeciales_idproyectosespeciales` tinyint NOT NULL,
...
--
-- Temporary table structure for view `view_pedidosproveedoresentradasalmacenfallados`
--

DROP TABLE IF EXISTS `view_pedidosproveedoresentradasalmacenfallados`;
/*!50001 DROP VIEW IF EXISTS `view_pedidosproveedoresentradasalmacenfallados`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE TABLE `view_pedidosproveedoresentradasalmacenfallados` (
  `idpedidosproveedoresentradasalmacenfallados` tinyint NOT NULL,
...
/*!50001 CREATE TABLE `view_pedidosproveedoresitemdetallereserva` (
  ...
  `item_iditem` tinyint NOT NULL,
  `nombre` tinyint NOT NULL,
  `idproyectosespeciales` tinyint NOT NULL,
  `proyectosespeciales` tinyint NOT NULL
) ENGINE=MyISAM */;
SET character_set_client = @saved_cs_client;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `view_porcentajesreservas` AS select `i`.`iditem` AS `iditem`,`i`.`editorial_ideditorial` AS `editorial_ideditorial`,`ihc`.`categoria_idcategoria` AS `categoria_idcategoria`,`p`.`bodega_idbodega` AS `bodega_idbodega`,`p`.`usergroups_user_id` AS `usergroups_user_id`,`p`.`proyectosespeciales_idproyectosespeciales` AS `proyectosespeciales_idproyectosespeciales`,`p`.`porcentaje` AS `porcentaje`,`p`.`prioridad` AS `prioridad` from (((`item` `i` join `item_has_categoria` `ihc` on((`ihc`.`item_iditem` = `i`.`iditem`))) join `categoria` `c` on(((`ihc`.`categoria_idcategoria` = `c`.`idcategoria`) and (`c`.`raiz` = 1)))) join `porcentajesreservas` `p` on(((`p`.`categoria_idcategoria` = `c`.`idcategoria`) and (`p`.`editorial_ideditorial` = `i`.`editorial_ideditorial`)))) order by `p`.`prioridad`,`p`.`porcentaje` desc,`p`.`usergroups_user_id` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
--
-- Final view structure for view `view_pedidosproveedoresentradasalmacenfallados`
--

/*!50001 DROP TABLE IF EXISTS `view_pedidosproveedoresentradasalmacenfallados`*/;
/*!50001 DROP VIEW IF EXISTS `view_pedidosproveedoresentradasalmacenfallados`*/;
/*!50001 SET @saved_cs_client          = @@character_set_client */;
/*!50001 SET @saved_cs_results         = @@character_set_results */;
/*!50001 SET @saved_col_connection     = @@collation_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `view_pedidosproveedoresentradasalmacenfallados` AS select `d`.`idpedidosproveedoresentradasalmacenfallados` AS `idpedidosproveedoresentradasalmacenfallados`,`d`.`idpedidosproveedoresentradasalmacen` AS `idpedidosproveedoresentradasalmacen`,`d`.`item_iditem` AS `item_iditem`,`d`.`cantidad` AS `cantidad`,`d`.`observaciones` AS `observaciones`,`i`.`nombre` AS `nombre`,`cc`.`idcondicioncomercial` AS `idcondicioncomercial`,`cc`.`nombre` AS `condicioncomercial` from ((`pedidosproveedoresentradasalmacenfallados` `d` join `item` `i` on((`i`.`iditem` = `d`.`item_iditem`))) join `condicioncomercial` `cc` on((`cc`.`idcondicioncomercial` = `d`.`condicioncomercial_idcondicioncomercial`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `view_pedidosproveedoresitemscorto` AS select distinct `view_pedidosproveedoresitemsagrupado`.`idpedidosproveedoresitems` AS `idppi`,`view_pedidosproveedoresitemsagrupado`.`pedidosproveedores_idpedidosproveedores` AS `idpp`,`view_pedidosproveedoresitemsagrupado`.`item_iditem` AS `iditem`,`view_pedidosproveedoresitemsagrupado`.`nombre` AS `nombre`,`view_pedidosproveedoresitemsagrupado`.`solicitado` AS `solicitado`,`view_pedidosproveedoresitemsagrupado`.`confirmado` AS `confirmado`,`view_pedidosproveedoresitemsagrupado`.`recibido` AS `recibido`,`view_pedidosproveedoresitemsagrupado`.`estado` AS `estado` from `view_pedidosproveedoresitemsagrupado` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `view_pedidosproveedoresitemdetallereserva` AS select `d`.`idpedidosproveedoresitemdetallereserva` AS `idpedidosproveedoresitemdetallereserva`,`d`.`usuarios_idusuarios` AS `usuarios_idusuarios`,`u`.`username` AS `username`,`d`.`bodega_idbodega` AS `bodega_idbodega`,`b`.`nombre` AS `nombrebodega`,`d`.`reservado` AS `reservado`,`ppi`.`solicitado` AS `solicitado`,`ppi`.`idpedidosproveedoresitems` AS `idpedidosproveedoresitems`,`ppi`.`item_iditem` AS `item_iditem`,`i`.`nombre` AS `nombre`,`pe`.`idproyectosespeciales` AS `idproyectosespeciales`,`pe`.`nombre` AS `proyectosespeciales` from (((((`pedidosproveedoresitemdetallereserva` `d` join `bodega` `b` on((`b`.`idbodega` = `d`.`bodega_idbodega`))) join `usergroups_user` `u` on((`u`.`id` = `d`.`usuarios_idusuarios`))) join `pedidosproveedoresitems` `ppi` on((`ppi`.`idpedidosproveedoresitems` = `d`.`pedidosproveedoresitems_idpedidosproveedoresitems`))) join `item` `i` on((`i`.`iditem` = `ppi`.`item_iditem`))) left join `proyectosespeciales` `pe` on((`pe`.`idproyectosespeciales` = `d`.`proyectosespeciales_idproyectosespeciales`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2013-01-18  3:25:37
