...
class ViewPedidosproveedoresitemscorto extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('idppi, idpp, iditem, solicitado, confirmado, recibido', 'numerical', 'integerOnly'=>true),
			array('nombre', 'length', 'max'=>512),
			array('estado', 'length', 'max'=>45),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('idppi, idpp, iditem, nombre, solicitado, confirmado, recibido, estado', 'safe', 'on'=>'search'),
		);
	}
